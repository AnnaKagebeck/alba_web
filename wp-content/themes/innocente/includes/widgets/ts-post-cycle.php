...
class TS_PostCycleWidget extends WP_Widget {
                  ...
                  <?php if ( $title )
						...
						<?php } else { ?>
							...
							<div class="<?php echo $boxslideshow; ?>">
								...
								<?php if($posttype=="display") { ?>
								<?php query_posts('&display-category='.$cat .'&showposts=-1');?>
								<?php } else { ?>
 ...
 <input class="widefat" id="<?php echo $this->get_field_id('limit'); ?>" name="<?php echo $this->get_field_name('limit'); ?>" type="text" value="<?php echo $limit; ?>" /></label></p>
		...
		<option value="testimonial" <?php echo ($posttype === 'testimonial' ? ' selected="selected"' : ''); ?>><?php _e('Testimonials','templatesquare');?></option>
