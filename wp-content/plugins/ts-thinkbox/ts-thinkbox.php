...
Description: Thinkbox is a simple way to show though from people, staffs, team and clients.
Version: 1.0
...
class TS_Thinkbox{
	...
	function __construct(){
		...
		register_activation_hook( __FILE__, array($this, 'ts_thinkbox_activation') );
		register_deactivation_hook( __FILE__, array($this, 'ts_thinkbox_deactivation') );
	...
	function ts_thinkbox_shortcode($atts){
		...
		$defattr = $this->defaultattr["plain"];		
		...
		$type = (in_array($type,$this->types))? $type : $defattr["type"];
		...
		$arrinclude = array();
		$arrinclude['post_type'] = $this->posttypename;
		if($postperpage>=0){
			...
			if (have_posts()){
				while ( have_posts() ){ 
					...
					if($cf_thumb!=""){
						...
						$cf_thumb = '<img src="'.$this->avatar.'" alt="" style="'.$widthheightimg.'"/>';
					}
	...
	function ts_thinkbox_slider_shortcode($atts){
		...
		$defattr = $this->defaultattr["slider"];		
		//make all shortcode attributes into single variable
		extract(shortcode_atts($defattr, $atts));
		...
		$sliderfx = (in_array($sliderfx,$this->effects))? $sliderfx : $defattr["sliderfx"];
				...
				if (have_posts()){
					while ( have_posts() ){ 
						...
						$htmlquote .= '<div class="ts-thinkbox-slider-quotecontent">';
							...
							$htmlquote .='<p class="ts-thinkbox-slider-quote">'.$text.'</p>';
	...
	function ts_thinkbox_action_init(){
		...
		wp_enqueue_script('cycle', plugin_dir_url( __FILE__ ).'/js/jquery.cycle.all.min.js', array('jquery'));
		wp_enqueue_script('thinkbox-slider', plugin_dir_url( __FILE__ ).'/js/ts-thinkbox-slider.js.php', array('jquery'));
		
		//Register and use this plugin main CSS
		wp_register_style('ts-thinkbox-main', plugin_dir_url( __FILE__ ).'css/ts-thinkbox.css', array(), $version . time());
	...
	function ts_thinkbox_filter_mce_plugin( $plugins ) {
		...
		$plugins['ts_thinkbox'] = plugin_dir_url( __FILE__ ) . 'js/ts-thinkbox-plugin.js';
		$plugins['ts_thinkbox_slider'] = plugin_dir_url( __FILE__ ) . 'js/ts-thinkbox-plugin.js';
		return $plugins;
	}
}
